...
class News_state extends State<News>{
  ...
  sort_items(){
    ...
    for (var post in widget.items){
      int index = 0;
      if(post['liked'].length >= widget.items[0]["liked"].length){
        sort_items.insert(0, post);
        print(post);
      } else {
        sort_items.add(post);
      }
      ...
      body: Container(
        ...
        child: Column(
          children: [
                    ...
                    Row(
                      children: [
                        GestureDetector(
                          ...
                          child: const Icon(Icons.add, color: Colors.redAccent, size: 32),
                        ),
                        const SizedBox(width: 12),
                        GestureDetector(
                          onTap: (){ Navigator.push(context, MaterialPageRoute(builder: (context) => Publisher_list()));  },
                          child: const Icon(Icons.supervised_user_circle_sharp, color: Colors.redAccent, size: 32),
                        ),
                      ],
                    ),
            ...
            Filter(handle_sort, widget.sort),
